...
WindowState = namedtuple('WindowState', ['keys', 'window', 'game', 'resources'])
...
class ResourceStore(dict):
    ...
    def __init__(self, *paths):
        super(ResourceStore, self).__init__()

        pyglet.resource.path = ['.'] + list(paths)
        ...
        for p in paths:
            for item in os.listdir(p):
                if item.endswith('png'):
                    ...
                    self[name] = pyglet.resource.image(item)
                    self[name].anchor_x = self[name].width // 2
                    self[name].anchor_y = self[name].height // 2
...
class Game(object):
    ...
    def __init__(self, first_scene_class, name, size, fullscreen=False):
        ...
        self.resource_store = ResourceStore('img')
...
class Scene(object):
    ...
    def res(self, name):
        return self.window_state.resources[name]

    def key(self, k):
        return self.window_state.keys[k]

    @property
    def width(self):
        return self.window_state.window.width

    @property
    def height(self):
        return self.window_state.window.height
